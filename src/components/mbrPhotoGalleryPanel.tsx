...
export default function MbrPhotoGalleryPanel({
  ...
  const loadCategoryPhotos = async () => {
    ...
    try {
      ...
      console.warn(`Could not load ${categoryTitle} photos:`, err);
      setMediaItems([]);
    } finally {
      setLoading(false);
    }
  ...
  if (!isOpen) {
    return <AdminComponentTag name="mbrPhotoGalleryPanel" />;
  }
  ...
  return (
    <>
      <AnimatePresence>
        <div key={`gallery-container-${categoryCd}-${subordinateId || 'main'}`} className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-6 md:p-10">
          {/* Backdrop Overlay */}
          <motion.div
          ...
          <motion.div
            ...
            className="relative w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl z-10 flex flex-col max-h-[92vh] sm:max-h-[90vh]"
          >
            {/* Top Accent Line */}
            <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 via-indigo-500 to-amber-500 opacity-80 z-20" />

            {/* Modal Header */}
            <div className="flex items-center justify-between px-3.5 sm:px-6 py-3 sm:py-4 border-b border-slate-800 bg-slate-900/95 text-white shrink-0 gap-2 relative">
              {/* Left Info / Title */}
              <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className="font-serif font-bold text-sm sm:text-base text-slate-100 truncate leading-tight">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {canUpload && (
                      <span className="text-[10px] text-slate-500 font-mono border-l border-slate-700 pl-2 hidden xs:inline-block">
                        Limit {maxPhotos}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
                ...
                {canUpload && (
                  <div className="hidden sm:flex items-center gap-2">
                    <label
                      ...
                      className={`inline-flex items-center gap-1.5 px-3.5 py-2 text-white rounded-xl text-xs font-bold font-sans transition-all cursor-pointer ${
                ...
                {canUpload && (
                  <div className="flex sm:hidden items-center gap-1.5">
                    <label
                      htmlFor={`gallery-upload-${categoryCd}`}
                      className={`inline-flex items-center gap-1 px-2.5 py-1.5 text-white rounded-xl text-xs font-bold font-sans transition-all cursor-pointer shrink-0 ${
                        mediaItems.length >= maxPhotos
                          ? 'bg-slate-700 text-slate-400 border border-slate-600 cursor-not-allowed opacity-60'
                          : uploading
                            ? 'bg-blue-600 opacity-70 cursor-wait'
                            : 'bg-blue-600 hover:bg-blue-700 active:scale-95 shadow-xs'
                      }`}
                      title={mediaItems.length >= maxPhotos ? `Max ${maxPhotos} reached` : `Upload Photo`}
                    >
                      {uploading ? (
                        <Loader2 className="w-3.5 h-3.5 animate-spin text-white" />
                      ) : (
                    ...
                    {mediaItems.length > 0 && currentPhoto && (
                      <button
                        ...
                        onClick={() => handleDeletePhoto(currentPhoto.mbrMediaId)}
                        disabled={deletingMediaId === currentPhoto.mbrMediaId}
                        className="p-1.5 bg-red-600/20 hover:bg-red-600/40 text-red-400 border border-red-500/30 rounded-xl transition-all cursor-pointer shrink-0"
                        title="Delete Current Photo"
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="p-1.5 sm:p-2 text-slate-400 hover:text-white rounded-xl hover:bg-slate-800 transition-colors cursor-pointer shrink-0"
            ...
            <div className="relative flex-grow flex items-center justify-center bg-black/60 p-4 min-h-[300px] md:min-h-[400px] overflow-hidden select-none">
              {loading ? (
                ...
                <>
                  ...
                  <motion.div
                    key={currentPhoto?.mbrMediaId ? `photo-${currentPhoto.mbrMediaId}-${currentIndex}` : `photo-idx-${currentIndex}`}
              ...
              <div className="px-6 py-3 border-t border-slate-800 bg-slate-950/80 shrink-0 overflow-x-auto">
                <div className="flex items-center justify-center gap-2.5 min-w-max mx-auto">
                  {mediaItems.map((item, idx) => {
                    ...
                    return (
                      <button
                        key={item.mbrMediaId ? `thumb-${item.mbrMediaId}-${idx}` : `thumb-idx-${idx}`}
                        type="button"
                        onClick={() => {
                          setIsEditingDescription(false);
                          setCurrentIndex(idx);
