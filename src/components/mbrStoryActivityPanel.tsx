...
import { Sparkles, Plus, Trash2, Edit3, Save, X, Loader2, AlertCircle, CheckCircle2, ShieldAlert, BookOpen, Images } from 'lucide-react';
...
export default function MbrStoryActivityPanel({ isSandbox = false, memberId, readOnly = false }: MbrStoryActivityPanelProps) {
  // --- STATE VARIABLES ---
      ...
      if (isSandbox) {
        ...
        try {
          ...
          if (dbActivities && dbActivities.length > 0) {
            ...
            setActivityList(SANDBOX_ACTIVITIES);
          } else {
            setActivityList([]);
          }
        } catch (err) {
        ...
        {!isEditing && (
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2.5 text-slate-400 hover:text-indigo-600 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl cursor-pointer transition-colors"
              title="Activity Photo Gallery"
            >
              <Images className="w-3.5 h-3.5 text-indigo-600" />
            </button>
            {/* Storybook Icon Button */}
            <button
              onClick={() => window.dispatchEvent(new CustomEvent('open-story-editor', { detail: { topicId: 'hobbies', topicTitle: 'Activities and Hobbies', componentName: 'sbMbrStryActivity' } }))}
