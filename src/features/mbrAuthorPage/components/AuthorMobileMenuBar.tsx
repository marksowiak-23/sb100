...
const AUTHOR_INDEX_TOPICS: StoryTopicItem[] = [
...
export default function AuthorMobileMenuBar({
  ...
  const currentTopic = AUTHOR_INDEX_TOPICS.find(
    (t) => t.id.toLowerCase() === activeSection?.toLowerCase()
  ) || AUTHOR_INDEX_TOPICS[0];
          ...
          {isOpen && (
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 mt-1">
                {AUTHOR_INDEX_TOPICS.map((topic) => {
                  const isActive = topic.id.toLowerCase() === activeSection?.toLowerCase();
