...
export default function ConnectionMobileMenuBar({
  ...
  const currentItem = menuItems.find((m) => m.id === activeSection || (activeSection === 'connections' && m.id === 'groups')) || menuItems[0];
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="overflow-hidden border-t border-[#EFECE7] dark:border-slate-800 pt-2.5"
            >
              {/* Top Level Item: My Connections (Above Manage Connections Header) */}
              {(() => {
                const myConnItem = menuItems.find((m) => m.id === 'my-connections');
                if (!myConnItem) return null;
                const isMyConnActive = activeSection === 'my-connections';
                const MyConnIcon = myConnItem.icon;

                return (
                  <div className="pb-2 mb-2 border-b border-[#EFECE7] dark:border-slate-800">
                    <button
                      type="button"
                      onClick={() => handleSelect('my-connections')}
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        <MyConnIcon className={`w-4 h-4 shrink-0 ${
                          isMyConnActive
                            ? 'text-amber-400 dark:text-amber-600'
                            : myConnItem.colorClass
                        }`} />
                        <span className="truncate">{myConnItem.label}</span>
                      </div>
              ...
              {/* Section Header: Manage Connections */}
              ...
              {/* Nested Items: Groups, Invitations, Requests */}
              <div className="flex flex-col gap-1.5 mt-1 pl-3">
                {menuItems.filter((item) => item.id !== 'my-connections').map((item) => {
                  const isActive = item.id === activeSection || (item.id === 'groups' && activeSection === 'connections');
